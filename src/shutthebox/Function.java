 ...
 */
 public static int[] turn(int[] tiles, String playerName, Dice[] dice) {
    ...
    // Main turn loop - continues until there are no more valid moves
    while (true) {
        ...
        // Calculate  and print the sum to the screen
        ...
        // Perform the move selected by the player
  ...
  */
    ...
    private static int[] selectAndPlay(int[] board, int total) {
        ...
        try {
            ...
            return selectAndPlay(board, total); // Retry in case of error
        }
    }
    ...
    private static boolean validateSelection(int[] board, int[] selected, int total) {
	    ...
	    /*
	     * Verify that all selected cards:
	     * - Exist (between 1 and 12)
	     * - Are not duplicates
	     * - Are available (not previously closed)
	     */
        boolean[] used = new boolean[13];  
        
        // tile-1 because the board uses indices 0-11 for tiles 1-12
        for (int tile : selected) {
            if (tile < 1 || tile > 12 || used[tile] || board[tile - 1] != tile) {
                return false; // Invalidation condition
            }
            used[tile] = true; // Mark this token as used
        }
